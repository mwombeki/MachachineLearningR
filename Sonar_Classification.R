...
model3 <- train(Class ~., Sonar, method = "ranger",  trControl = myControl)
...
model3 <- train(Class ~., Sonar, method = "ranger",  tuneGrid = myControl2)
model3
plot(model3)

#Using RandomFrorrest with a tunelenth/default
model2 <- train(Class ~., Sonar, method = "ranger", tuneLength = 10 )
plot(model2)
model2

#using glmnet with trainControl
model4 <- train(Class ~., Sonar, method = "glmnet", trControl = myControl)
model4
...
tuneGridx = expand.grid(alpha = 0:1,
                       lambda = seq(0.0001, 1, length = 20))

model5 <- train(Class ~., Sonar, method = "glmnet", tuneGrid = tuneGridx)
print(model5)
